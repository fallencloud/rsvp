class InviteeClass {
  ...
  //loads invitees
  loadInvitees() {
    let inviteeHtml = this.invitees.reduce(
      (html, invitee, index) =>
        (html += this.generateInviteeHtml(invitee, index)),
      ""
    );
  ...
  generateInviteeHtml(invitee, index) {
    return `
    <li>${invitee.name}<label>Confirmed <input type="checkbox" ${
      invitee.isConfirmed ? "checked" : ""
    }onChange="invitee.toggleInviteeStatus(${index})"></label><button onClick="invitee.deleteInvitee(event, ${index})">remove</button></li>
    `;
